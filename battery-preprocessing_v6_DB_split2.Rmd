...
oddeven <- 0 #set to zero for full dataset; to 1 for odds, and to 2 for evens
#NB we want to use exclusion criteria from full dataset for the split half as well, and so we need to run the script with oddeven set to zero before using it for split

#Demographics not needed for odd/even runs
if (oddeven==0){
#Reading from OSF - this only works if files are public
demo_dat <- read.csv('https://osf.io/7smj6/download',stringsAsFactors = F)

# relabel 
demo_dat$origID <- demo_dat$Participant.Public.ID
demo_dat$origID <- as.factor(demo_dat$origID)
# count and print subjects
nsub <-length(levels(demo_dat$origID))
print(paste0("subjects: ", nsub))

# create data frame
allsum <- data.frame(matrix(ncol = 7, nrow = nsub))
  colnames(allsum) <- c("origID", "age", "gender", "footedness", "handedness", "bilingual", "strongest.language")
myrow <- 0 # start row counter
for (i in 1:nsub) { # loop through subjects
  subname <- levels(demo_dat$origID)[i] # find origID subject
  myrows <- which(demo_dat$origID==subname) # select rows for this subject
  tmp <- data.frame(demo_dat[myrows,])

    myrow <- i

    allsum$origID[myrow] <- subname
...
miles_dat$test_origID <- "miles"
porta_dat$test_origID <- "porta"
# merge the two
sight_dat <- rbind(miles_dat, porta_dat)
# relabel 
sight_dat$origID <- sight_dat$Participant.Public.ID
sight_dat$origID <- as.factor(sight_dat$origID)

# count and print subjects
nsub <-length(levels(sight_dat$origID))
print(paste0("subjects: ", nsub))

# create data frame
sightedness <- data.frame(matrix(ncol = 3, nrow = nsub))
  colnames(sightedness) <- c("origID", "miles", "porta")
myrow <- 0 # start row counter
for (i in 1:nsub) { # loop through subjects
  subname <- levels(sight_dat$origID)[i] # find origID subject
  myrows <- which(sight_dat$origID==subname) # select rows for this subject
  tmp <- data.frame(sight_dat[myrows,])

    myrow <- i

    sightedness$origID[myrow] <- subname
...
allsum <- merge(allsum, sightedness, by= "origID",all=TRUE)
...
# recode variables
EHI$origID <- EHI$Participant.Public.ID
EHI$origID <- as.factor(EHI$origID)

# reduce data
# new data
meaningful <- c("origID", "Question Key", "Response") # select wanted columns
...
# re-reduce data
# new data
meaningful <- c("origID", "right_hand", "left_hand") # select wanted columns
c<-which(names(EHI) %in% meaningful) #find colnumbers of unwanted
EHI <-EHI[,c] #remove unwanted columns
EHI <- na.omit(EHI) # remove NAs
...
# create hand data 
nsub <-length(levels(EHI$origID))
EHI2 <- data.frame(matrix(ncol = 3, nrow = nsub))
  colnames(EHI2) <- c("origID", "right_hand", "left_hand")


for (i in 1:nsub) { # loop through subjects
  myrow <- i
  subname <- levels(EHI$origID)[i] # find origID subject
  myrows <- which(EHI$origID==subname) # select rows for this subject
  tmp <- data.frame(EHI[myrows,])
  
    EHI2$origID[myrow] <- subname # add row for subject
...
for (i in 1:nrow(allsum)){
  thissub <- allsum$origID[i]
  w<-which(EHI2$origID==thissub)
...
eplot
#This is not perfect, but gives and ID to check EHI is valid etc.
...
# rename variables
lexTALE$origID <- lexTALE$Participant.Public.ID
lexTALE$accuracy <- lexTALE$Correct
lexTALE$condition <- lexTALE$ANSWER
# code as factors
lexTALE$origID <- as.factor(lexTALE$origID)
...
lexTale_score <- data.frame(matrix(ncol = 4, nrow = nsub))
  colnames(lexTale_score) <- c("origID", "Word", "nonWord", "lexTALE")

for (i in 1:nsub) { # loop through origIDs
  subname <- levels(lexTALE$origID)[i] # find origID subject
  myrows <- which(lexTALE$origID==subname) # select rows for this subject
  tmp <- data.frame(lexTALE[myrows,])

    myrow <- i

    lexTale_score$origID[myrow] <- subname
...
# plot
x<-ggplot(lexTale_score, aes(x=lexTALE, y=origID, fill=origID)) +
...
# now create a data frame only for index 
lexTALE <- dplyr::select(lexTale_score, "origID", "lexTALE")
# now merge with allsum
allsum <- merge(allsum, lexTALE, by= "origID",all=TRUE)
...
```{r readraw,include= FALSE, warning=FALSE}
  #reading direct from OSF is temperamental
  #Often gives error message if run from chunk, but line by line is OK. Grrr.
  readosf<-0
  if(readosf==1){
  ...
  if(readosf==0){
    all_dat1=read.csv("./data/day1_combined.csv",na.strings = "NA",stringsAsFactors=F)
      all_dat2=read.csv("./data/day2_combined.csv",na.strings = "NA",stringsAsFactors=F)
  }
...
# write as factors
alldat$origID <- as.factor(alldat$Participant.Public.ID)
alldat$subject<-as.numeric(alldat$origID)
...
##################################################################################
#      MODIFICATION FOR ODD AND EVEN ANALYSIS
##################################################################################
...
```{r reuse.allsum}
#for odd and even subsets, read in original allsum and select demographic and exclusion cols to keep
if (oddeven>0){
  allsum<-read.csv("./data/allsum_out.csv",stringsAsFactors=F)
  keepcols<-c("subject" , "origID","age" ,"gender","footedness" ,"handedness","bilingual", "strongest.language","handpch","genderpch" ,"miles", "porta","index_EHI" ,"lexTALE", "lexOut","exclude","exRDT"      )
  allsum<-allsum[,keepcols]
}
head(allsum)
...
```{r subnums}
##################################################################################
#find subject number corresponding to orig ID

allsum$subject<-NA
for (i in 1:nrow(allsum)){
  thisID<-allsum$origID[i]
  w<-which(alldat$origID == thisID)
  allsum$subject[i]<-alldat$subject[w[1]]
}
...
```{r marktecherr}
Nfortech<-80 #need at least this N trials
if(oddeven>0){Nfortech<-40} #half the number for split half runs
techerr<-table(dichotic_dat2$subject)
te<-which(techerr<Nfortech)
techerr<-techerr[te] #subject numbers for those with few trials.
techerr<-as.numeric(names(techerr))
w<-which(allsum$subject%in% techerr)
allsum$exclude[w]<-4 #exclude for tech error
...
#only plot/t-test included cases
...
#for splithalves read in the original RDT file for all trials and then split
#This means we keep the outlier designation as before
if(oddeven>0){
  tempRDT1<-read.csv("./data/myRDTdat1.csv",stringsAsFactors=F)
  tempRDT2<-read.csv("./data/myRDTdat2.csv",stringsAsFactors=F)
  if(oddeven==1){
  RDT_dat1<-tempRDT1[tempRDT1$Trial%%2==1,] #odd trials only
  RDT_dat2<-tempRDT2[tempRDT2$Trial%%2==1,] #odd trials only
  }
    if(oddeven==2){
  RDT_dat1<-tempRDT1[tempRDT1$Trial%%2==0,] #even trials only
  RDT_dat2<-tempRDT2[tempRDT2$Trial%%2==0,] #even trials only
  }
}
```
...
w1<-which(nr1$p.corr<75)
w2<-which(nr2$p.corr<75)
...
w1a<-which(wr1$p.corr<50)
w2a<-which(wr2$p.corr<50)
...
origdat$X1 <- origdat$RT
measure <- 'RT'
#need to exclude those who don't have RT values on both L and R
w<-which(is.na(origdat$X1))
if (length(w)>0){
  takeout<-unique(origdat$subject[w])
  ww<-which(origdat$subject %in% takeout)
  origdat<-origdat[-ww,]
}
dopirate(origdat,task,measure)
}
...
write.csv(allsum, paste0("./data/LIs",oddeven,".csv"))
```
...
allsum <- merge(allsum, complete.day2, by= "subject", all= TRUE)
  ...
  allsumex<-allsum #make a copy that has the exclusions from the whole dataset
  write.csv(allsum,'./data/allsum_out.csv',row.names=F)
}
LIname<-paste0('./data/LIs',oddeven,'.csv')
LIcols<-c("subject","handpch", "Day1_CF_acc_LI", "Day2_CF_acc_LI",
          "Day1_DL_acc_LI" ,"Day2_DL_acc_LI" ,
          "Day1.RDT_RT_LI" ,"Day2.RDT_RT_LI")

write.csv(allsum[,LIcols],LIname,row.names=F)

zname<-paste0('./data/LIzs',oddeven,'.csv')
LIzcols<-c("subject","handpch", "Day1_CF_acc_Z", "Day2_CF_acc_Z",
          "Day1_Dich_acc_Z" ,"Day2_Dich_acc_Z" ,
          "zlat_RDT_1" ,"zlat_RDT_2")

write.csv(allsum[,LIzcols],zname,row.names=F)

#we need to preserve the RDT files that have the outliers marked as NA
if(oddeven==0){
  write.csv(RDT_dat1,'./data/myRDTdat1.csv',row.names=F)
  write.csv(RDT_dat2,'./data/myRDTdat2.csv',row.names=F)
}
