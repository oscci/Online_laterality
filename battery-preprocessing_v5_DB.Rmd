...
demo_dat <- read.csv('./data/Demographics.csv',stringsAsFactor=F)
...
# Set directory
for (d in 1:2){
  data_file <- "./data/combined/day1_combined.csv" #day 1 data
...
# add log RT
RDT_df1$logRT <- log10(RDT_df1$RT)
RDT_df2$logRT <- log10(RDT_df2$RT)
...
# log-RT
origdat$X1 <- origdat$logRT
mycolname<-"Day1.RDT_logRT_LI"
if(d==2){mycolname<-"Day2.RDT_logRT_LI"}
polarity <- (-1)
mycutoff <- 0
allsum <- makeLI(allsum,origdat,task,mycolname,polarity,mycutoff)
}
...
t.test(partsum$Day1.RDT_logRT_LI)
t.test(partsum$Day2.RDT_p.corr_LI)
t.test(partsum$Day2.RDT_RT_LI)
t.test(partsum$Day2.RDT_logRT_LI)
...
cor3<-cor(partsum$Day1.RDT_logRT_LI,partsum$Day2.RDT_logRT_LI,use='complete.obs')
# acc plot
...
# log plot
plot(partsum$Day1.RDT_logRT_LI,partsum$Day2.RDT_logRT_LI,main='Rhyme detect: LI for log-RT')
abline(v=0)
abline(h=0)
text(2,1,paste0('r = ',round(cor3,3)))
```
...
(Some of these tests etc have been incorporated in analyses above, so can be pruned from script)
...
# first, describe age for the whole sample and plot
plot(data=allsum, index_EHI ~ age,col=handpch,xlim=c(18,50),pch=(14+allsum$genderpch))

allsum$handfac <- as.factor(allsum$handedness)
allsum$genderfac <- as.factor(allsum$genderpch)
levels(allsum$genderfac)<-c('male','female')

a <- allsum %>%
  group_by(genderfac, handfac) %>%
    summarize(n = n())


ggplot(data=a, aes(x=handfac, y=n, fill= handfac)) +
  geom_bar(stat="identity")+
  geom_text(aes(label=n), vjust=1.6, color="white", size=3.5)+
  theme_classic() + facet_wrap(.~ genderfac) + ggtitle("Gender by handedness") +   ylab("Count") + xlab("")

# third, show footedness
a <- allsum %>%
  group_by(handfac, footedness) %>%
    summarize(n = n())
ggplot(data=a, aes(x=handfac, y=n, fill= handfac)) +
  geom_bar(stat="identity")+
  geom_text(aes(label=n), vjust=1.6, color="white", size=3.5)+
...
a <- allsum %>%
  group_by(bilingual,handfac) %>%
    summarize(n = n())


# fifth, show miles
a <- allsum %>%
  group_by(miles, handfac) %>%
    summarize(n = n())
ggplot(data=a, aes(x=handfac, y=n, fill= handfac)) +
  geom_bar(stat="identity")+
  geom_text(aes(label=n), vjust=1.6, color="white", size=3.5)+
...
ggplot(allsum, aes(x = handfac, y = lexTALE)) +
  geom_pirate(aes(colour = handfac), bars = FALSE) + theme_classic() + 
...
#check lateralisation with single group t vs zero
for (i in colnums){
  print(names(allsum)[i])
