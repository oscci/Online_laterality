...
https://pubmed.ncbi.nlm.nih.gov/18050003/
...
# relabel 
demo_dat$subject <- demo_dat$Participant.Public.subject
...
# relabel 
sight_dat$subject <- sight_dat$Participant.Public.subject
...
# recode variabels
EHI$subject <- EHI$Participant.Public.subject
...
Create a simple numerical ID to make life easier if we need to identify datapoints which are odd in some way. Ensure this is consistent across all_dat1 and all_dat2 and allsum.
We will rename the current subject variable so we can reuse this one  for all future computations.
...
#Now extract correct RTs for each subject to compute limits for outliers
  ...
  for (i in 1:nsub) { # loop through subjects
  subname <- levels(origdat$subject)[i] # find subject 
...
myrow <- 0 # start row counter
for (i in 1:nsub) { # loop through subjects
  subname <- levels(origdat$subject)[i] # find subject subject
  myrows <- which(origdat$subject==subname) # select rows for this subject
...
allsum<-allsum[,-w]
#now add this LI
allsum <- merge(allsum, Lsubjectf, by= "subject")
...
# now merge with allsum
# First check if this column already exists and delete it if so
w<-which(colnames(allsums)==mycolname)
allsums<-allsums[,-w]
...
# count subjects
nsub <-length(levels(CF_dat_all$subject))
...
#Most have 96 accepted trials in CF_dat2. A few have zero - these seem to be ones that had RTs < 200, and may have been responding rapidly at random.
#One person has 192 trials - seems to have had whole thing presented twice.
...
First, we judge participants' ability to correctly identify emotions using the "CF_same" dataframe. Participants are removed if their performance is below 75% as this indicates poor ability to identify emotions, which will inevitably influence performance on the Chimeric Face Task.
...
emotion_mean <- aggregate(FUN= mean, data= CF_same, Correct~ subject)
# add this to allsum 
allsum <- merge(allsum, emotion_mean, by= "subject")
}

w<-which(colnames(allsum) %in% c("Emot.recog1","Emot.recog2"))
if (length(w)>0){
...
plot(jitter(allsum$Emot.recog1,10),jitter(allsum$Emot.recog2,10),pch=3,cex=.6,xlab='Nonchimeric emotion recog 1',ylab='Nonchimeric emotion recog 2')
abline(h=.75)
abline(v=.75)
...
print(paste('N excluded because < 75% on at least one trial:',length(w)))
```
...
chim_dat1 <- chim_dat1[(chim_dat1$subject %in% inclsubs),]
chim_dat2 <- chim_dat2[(chim_dat2$subject %in% inclsubs),]
...
origdat <- chim_dat1
measure <- 'Accuracy'
origdat$X1 <- chim_dat1$accurate #we used a dummy column name so we can vary the data that are used in the pirate plot etc with the dopirate function
...
cor1 <- cor(allsum$Day1_CF_acc_LI,allsum$Day2_CF_acc_LI)
...
cor2<-cor(allsum$Day1_CF_RT_LI,allsum$Day2_CF_RT_LI)
...
#allsum <- allsum[allsum$subject != remove.p,]
...
#We first need to remove rows where RT is coded NA - these seem cases where false recording led to excess trials with no RT
w<-which(is.na(RDT_dat1$RT))
RDT_dat1<-RDT_dat1[-w,]
origdat <- RDT_dat1

latlist <- c("Left","Right") #names of factor levels
RDT_df1 <- make.df(origdat,varlist,latlist)
w<-which(is.na(RDT_dat2$RT))
RDT_dat1<-RDT_dat2[-w,]
origdat <- RDT_dat2
