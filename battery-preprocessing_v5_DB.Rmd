...
for (i in 1:nsub) { # loop through subjects
  ...
  for (j in 1:2) { #make row for means for each side for this subject
    myrow <- myrow+1
...
# add this to allsum 
allsum <- merge(allsum, emot_corr_Diff, by= "subject")
}
...
plot(jitter(allsum$Emot.corr.same1,10),jitter(allsum$Emot.corr.same2,10),pch=3,cex=.6,xlab='Nonchimeric emotion recog 1',ylab='Nonchimeric emotion recog 2')
abline(h=.75)
abline(v=.75)
