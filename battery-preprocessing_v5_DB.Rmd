...
09 June 2020: AP made changes to calcultaions of rhyme
...
for (d in 1:2){
  ...
  if (d ==2){
    data_file <-  "https://osf.io/ebm6s/download" # day 2 data
  }
...
for (d in 1:2){
  ...
  aggdata <- aggregate(thisdat$RT,by=list(thisdat$side,thisdat$item,thisdat$Participant.Public.ID),
                       FUN=mean,na.rm=TRUE)
  aggdata$subject <- aggdata$Participant.Public.ID
...
# add t score to the data
t_dat <- select(pairedt, -3, -5)
t_dat <- na.omit(t_dat)
# merge to allsum for analysis
allsum <- merge(allsum, t_dat, by.x = "origID", by.y = "subject", all.x=T)
...
In our pre-reg, we also specified that we would use logRT. So we add this in here.

```{r logRT analysis}
# now add the logRT to original output
# first relabel df
RDT_logdat1 <- RDT_dat1
RDT_logdat2 <- RDT_dat2
# now log the RT
RDT_logdat1$RT <- log(RDT_dat1$RT)
RDT_logdat2$RT <- log(RDT_dat2$RT)
# Now use generic function to remove outliers.
# Here we keep the 1.65 of the H&I removal. 
# BUT we now specify log 200 for lower cutoff to move onto the same scale
RDT_logdat1<-detect.outliers(RDT_logdat1,log(200),1.65) #numbers specify min RT and 
RDT_logdat2<-detect.outliers(RDT_logdat2,log(200),1.65) #numbers specify min RT and zscore for Hoaglin-Iglewicz respectiv
# We create logRT for day1 and day 2
varlist <-   c("subject", "side", "accurate", "RT") #need to be in this order, ie sub, side, acc and RR
latlist <- c("left","no rhyme","right") #names of factor levels
nloop<-length(latlist)
RDT_log_df1 <- make.df(RDT_logdat1,varlist,latlist,nloop)
RDT_log_df2 <- make.df(RDT_logdat2,varlist,latlist,nloop)
# remove NaN
is.nan.data.frame <- function(x)
  do.call(cbind, lapply(x, is.nan))
RDT_log_df1[is.nan(RDT_log_df1)] <- NA
RDT_log_df2[is.nan(RDT_log_df2)] <- NA
# now let's plot the log data
for (d in 1:2){
origdat <-RDT_log_df1[RDT_log_df1$subject %in% incRDT,]
if (d==2)
{origdat <-RDT_log_df2[RDT_log_df2$subject %in% incRDT,]}
origdat <-origdat[origdat$side %in% c('left','right'),]
# RT
origdat$X1 <- origdat$RT
measure <- 'log-RT'
...
# make LI for log RT
# Because outliers will already be marked in allsum, we won't bother going through this again. 
# We will intead add straight into the data frame. 
#now create LI
for (d in 1:2){
origdat <-RDT_log_df1[RDT_log_df1$subject %in% incRDT,]
if (d==2)
...
mycutoff <- 0
origdat$X1 <- origdat$RT
mycolname<-"Day1_RDT_logRT_LI"
if(d==2){mycolname<-"Day2_RDT_logRT_LI"}
polarity <- (1)
allsum <- makeLI(allsum,origdat,task,mycolname,polarity,mycutoff)
}
# plot log RT
cor1<-cor(allsum$Day1_RDT_logRT_LI,allsum$Day2_RDT_logRT_LI,use='complete.obs')
plot(allsum$Day1_RDT_logRT_LI,allsum$Day2_RDT_logRT_LI,main='Rhyme detect: LI for log RT')
abline(v=0)
abline(h=0)
text(-1,1,paste0('r = ',round(cor1,3)))
# FINALLY, make t-test for logRT
#remove 'no rhyme' trials
for (d in 1:2){
  thisdat<-RDT_logdat2[RDT_logdat2$side %in% c('left','right'),]
  if(d==1){
    thisdat<-RDT_logdat1[RDT_logdat1$side %in% c('left','right'),]
  }
...
t_dat <- na.omit(t_dat)
# relabel for merging
names(t_dat)[names(t_dat) == "t1"] <- "logt1"
names(t_dat)[names(t_dat) == "t2"] <- "logt2"
# merge to allsum for analysis
allsum <- merge(allsum, t_dat, by.x = "origID", by.y = "subject", all.x=T)
...
Even when log-transforming, things don't look any better.
...
complete.day2 <- select(complete.day2, -2, -3, -4, -5)
...
plot.data <- plot.data %>%
  mutate(remove= if_else(plot.data$task== "RD" & plot.data$p.corr < .75,1,
                         if_else(plot.data$accurate <= 10,1,0)))
plot.data <- plot.data[plot.data$remove== 0,]
...
CF.cor.plot <- ggplot(abs(CF.corrs), aes(x=trial, y=cor)) +
...
Based on a couple of rounds with this, removing a lower limit on trials correct here is the only way to examine if less than 1 block would be enough to achieve r= .75. From this analysis, it appears that 25 trials would be sufficient to run the chimeric faces wirth reasonable reliability. Three repetitions of all combinations would lead to 48 trials (36 with different emotions in each hemifield, 12 same-same trials). This would ensure full balancing as 25 items would not allow an equal presentation of each of the four emotions across hemifaces.
...
DL.cor.plot <- ggplot(abs(DL.corrs), aes(x=trial, y=cor)) +
  geom_hline(yintercept=.65, linetype="dashed", color = "red") +
  geom_line(size=1, alpha=0.8) + ylim(0, 1) + theme_classic() +
  xlab("No. of trials") + ylab("Test-retest reliability") + ggtitle("A") + theme(plot.title = element_text(hjust = 0.5))
# print number of trials needed
print(paste0("Number of dichotic required for at least .65 test-retest: ", min(DL.corrs[DL.corrs$cor >= .65,]$trial)))
print(paste0("Number of dichotic required for at least .75 test-retest: ", min(DL.corrs[DL.corrs$cor >= .75,]$trial)))
