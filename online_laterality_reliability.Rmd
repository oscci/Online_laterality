...
For the rhyme task, we pre-registered that we would calculate LIs based on response time (RT) data, as accuracy during piloting was at ceiling. We specified that we would calculate LIs in two ways: (1) in the traditional fashion (100 x (Right – Left)/(Right + Left)); (2) to correspond to a *t*-value. This was achieved by conducting a *t*-test for each participant where accurate RTs are the dependent variable and visual field is the independent variable. This latter measure is highly correlated with the conventional laterality index but has the advantage that it can be used to identify participants who show a statistically significant RT advantage for one side, i.e. where the *t*-value on a 2-sided test has *p*< .05. Based on considerations from pilot data, we adopted log-transformation of RTs before computing *t*-values. 

For the dichotic listening and chimeric faces, we specified that LI calculated would be based on trials in which participants correctly identified a stimulus. The count of correct responses that corresponded to each side was used to generate an accuracy LI in two ways: (1) in the traditional fashion (100 x (Right – Left)/(Right + Left)); (2) as a *z*-statistic for each participant, using z = (pR-.5)/sqrt(pR x pL/n), where pR is the proportion of correct responses in the RVF, pL is proportion of L responses in the LVF, and n is total LVF and RVF responses. This was analogous to the individual t-scores generated for RTs in the rhyme decision task, in allowing us to identify individuals with a significant bias to one side. Finally, LIs for the finger tapping task were calculated in the same manner, but using the number of sequences generated by the left and right hand as opposed to the proportion of correct responses in each visual field.
...
To address hypothesis 2, that extreme left-handers will be more likely to show atypical laterality than right-handers, we categorised participants as extreme left-handers if they scored less than -90 and right-handers if they scored more than 90 of the Edinburgh Handedness Inventory. Atypicality was first defined in participants whose *t*- or *z*-score was significantly lateralised in the opposite direction from usual. If there were less than 20 cases defined as atypical using this method, we adjusted the cut-off on that task until we acquired 20 atypical participants. Once typicality was defined, we first analysed atypicality on the Rhyme Decision and Chimeric Faces task. Participants were categorised into four groups: typical on both; atypical on Rhyme Decision only; atypical on Chimeric Faces only, and atypical on both. A chi-square test was then used to test for association with the number of people in each of the four groups and the handedness classification from the Edinburgh Handedness Inventory. This was then be repeated for the Dichotic Listening and the Chimeric Faces tasks. A Bonferroni correction was applied to the outcome of each of the two chi-squared tests.
...
# read CF data
for_figs <- read.csv("for_figs.csv", na.strings = "NA")
...
<!---These figures aren't so good for showing laterality because they don't show how the L and R scores relate in individuals. Will try an alternative approach.--->

```{r altplotraw,echo=FALSE,include=FALSE, comment=FALSE, warning=FALSE}

makerawplot <- function(file1,file2,mysession,myname,mylim){


tempL<-filter(file1,Session==mysession)
tempR<-filter(file2,Session==mysession)
meanL<-mean(tempL$tempcol,na.rm=T)
meanR<-mean(tempR$tempcol,na.rm=T)
tempLR<-as.data.frame(cbind(tempL$tempcol,tempR$tempcol))
colnames(tempLR)<-c('Left','Right')
mytitle<-paste0(myname,"\n Session",mysession)
myg <- ggplot(tempLR, aes(x=Left, y=Right)) + geom_point(size=.5) +
...
RD.short.dat$tempcol<-RD.short.dat$RT
file1 <-filter(RD.short.dat,side=='left')
file2 <-filter(RD.short.dat,side=='right')
myname<-'Rhyme judge RT'
mysession<-1
mylim<-c(500,1500)
myg1<-makerawplot(file1,file2,mysession,myname,mylim)
myg2<-makerawplot(file1,file2,2,myname,mylim)

DL.short.dat$tempcol<-DL.short.dat$accurate
file1 <-filter(DL.short.dat,side=='left')
file2 <-filter(DL.short.dat,side=='right')
myname<-'Dichotic accuracy'
mysession<-1
mylim<-c(0,100)
myg3<-makerawplot(file1,file2,mysession,myname,mylim)
myg4<-makerawplot(file1,file2,2,myname,mylim)

CF.short.dat$tempcol<-CF.short.dat$accurate
#there are 96 stimuli with different faces, so 48 is pt of equality if all trials included
file1 <-filter(CF.short.dat,side=='left')
file2 <-filter(CF.short.dat,side=='right')
myname<-'Chimeric faces accuracy'
mylim<-c(0,96)
mysession<-1
myg5<-makerawplot(file1,file2,mysession,myname,mylim)
myg6<-makerawplot(file1,file2,2,myname,mylim)

finger.short.dat$tempcol<-finger.short.dat$sequences
file1 <-filter(finger.short.dat,side=='left')
file2 <-filter(finger.short.dat,side=='right')
myname<-'Finger tap sequences'
mysession<-1
mylim<-c(0,175)
myg7<-makerawplot(file1,file2,mysession,myname,mylim)
myg8<-makerawplot(file1,file2,2,myname,mylim)

ggarrange(myg1,myg2,myg3,myg4,myg5,myg6,myg7,myg8, ncol = 2, nrow = 4)
ggsave(
  "all.raw.png",
  width = 5, height = 8,
  dpi = 300
...
### Laterality indices: means and test-retest reliability

Accuracy on the rhyme task was high  (*M*= `r round(mean(RD.short.dat$p.corr, na.rm= TRUE),1)`%, *SD*= `r round(sd(RD.short.dat$p.corr, na.rm= TRUE),2)`%), and only correct response times were used for computing a laterality index. Average reaction times on this task are shown in Figure 3 as vertical and horizontal lines in a scatterplot showing right vs left visual field, separately for each session. A sloping dotted line shows the point where left and right RTs are equal; if there is faster performance for the right visual field, as predicted, then points should cluster below this line. Similar scatterplots are shown for selection of left and right responses for dichotic listening and chimeric faces. Note that for these two tasks, either side can give a correct response; the negative correlation between the two sides reflects the fact that the number of selections on the left will be inversely related to the number of correct responses on the right. For these tasks it is evident by inspection that most responses for dichotic listening cluster above the dotted line, indicating a right ear advantage, whereas most responses on chimeric faces cluster below the dotted line. Finally, we show the number of correct sequences with each hand on the finger-tapping task. Here, as expected, inspection of the data suggests more sequences with the right than the left hand on average.
...
The count of correct answers on the dichotic listening and chimeric faces tasks are also shown in Figure 3, along with the number of sequences generated by the left and right hand. For all tasks, we computed LIs in the typical way: (100 x (Right – Left)/(Right + Left)). It is important to note, however, that we present LIs such that they are reported to be consistent with lateralisation at the brain level (i.e. by multiplying LIs by -1). A negative LI indicates left lateralisation at the brain level while a positive LI indicates right lateralisation at the brain level. For the rhyme decision task, we then computed an LI using a *t*-test for individual participants based on log-transformed response time data. For the remaining tasks (which made use of count data) we computed a *z*-statistic for each participant based on proportions. 

For each measure, LIs and their test-retest correlation coefficients are reported in Table 2. Note that these are computed so that in each case a negative LI indicates left lateralisation (faster RTs or greater accuracy) at the brain level while a positive LI indicates right lateralisation at the brain level. In addition, Table 1 includes t-values and corresponding *p*-values for one-sample *t* tests for each measure, to test whether the mean LI departed significantly from zero. In addition Shapiro-Wilks test results are shown, to test departurs from normality.  As the distributions of most LIs were non-normal, Spearman's rank correlations were used to estimate the test-retest reliability. Confidence intervals were estimated using the *spearman.ci()* function from the RVAideMemoire package (version 0.9-75; Herv, 2020) with 10,000 iterations.  Furthermore, LIs for each task across the two sessions were lateralised at the group level (all *p*s< .001). Specifically, the rhyme decision, dichotic listening, and finger tapping tasks were all lateralised to the left hemisphere (showing a left hemisphere advantage). The chimeric faces task was lateralised to the right hemisphere. A highly similar pattern of results emerges for the *t*- and *z*-score measures, where each is lateralised at the population level (all *p*s< .001). For all but the rhyme decision task, we obtained good test-retest reliability that exceeded our pre-registered threshold of *r*= .65.The scatterplots depicting test-retest reliability are shown in Figure 4.
