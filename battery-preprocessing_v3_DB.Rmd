...
# Outlier removal put in separate function, as needed to reuse when considering t-test
...
CF_dat2 <- procdata(origfile,wanted,task)
...
CF_dat2 <- remove.outliers(CF_dat2,200,2.2)
...
for (i in 1:nsub) { # loop through subjects
  ...
  for (j in 1:2) {
    ...
    temp_dat$accurate[myrow] <- 100*mean(tmp1$Correct,na.rm=TRUE) # add row for accurate responses - this has been modified to give % correct
...
plot(demographics$Day1_CF_RT_LI,demographics$Day2_CF_RT_LI,main='Chimeric LI RT')
abline(v=0)
abline(h=0)
...
# new data
wanted <- c("subject", "Correct", "RT", "ANSWER","Response") # select wanted columns
...
w<-which(RDT_df1$accurate<60)
if (length(w)>0)
{RDT_df1$accurate[w]<-NA
RDT_df1$RT[w]<-NA
}
w<-which(RDT_df2$accurate<60)
if (length(w)>0)
...
t.test(demographics$RDT_acc_LI)
...
cor1<-cor(demographics$Day1.RDT_acc_LI,demographics$Day2.RDT_acc_LI,use='complete.obs')
cor2<-cor(demographics$Day1.RDT_RT_LI,demographics$Day2.RDT_RT_LI,use='complete.obs')
plot(demographics$Day1.RDT_acc_LI,demographics$Day2.RDT_acc_LI,main='Rhyme detect: LI for accuracy')
abline(v=0)
abline(h=0)
...
wanted <- c("subject", "Response", "display") # select wanted columns
...
colnames(finger.df)<-c('subject','Leasy1','Reasy1','Lhard1','Rhard1',
                       'Leasy2','Reasy2','Lhard2','Rhard2')
for (n in 1:nsub){
...
finger.left <- finger.df[,c(1,2,4,6,8)]
finger.right <- finger.df[,c(1,3,5,7,9)]
finger.left$side<-'Left'
finger.right$side <-'Right'
colnames(finger.right)<-c('subject','easy1','hard1','easy2','hard2','side')
...
mycolname<-"Day1_finger.easy_LI"
mycutoff <- 10 #NB! need to check against prereg. I think this is N correct, rather than percentage. People with avg scores below this value will be excluded.
polarity<- (1)#so LI is positive in predicted direction
demographics <- makeLI(demographics,origdat,task,mycolname,polarity,mycutoff)

origdat$X1 <- origdat$hard1
mycolname<-"Day1_finger.hard_LI"
demographics <- makeLI(demographics,origdat,task,mycolname,polarity,mycutoff)

origdat$X1 <- origdat$easy2
mycolname<-"Day2_finger.easy_LI"
demographics <- makeLI(demographics,origdat,task,mycolname,polarity,mycutoff)

origdat$X1 <- origdat$hard2
mycolname<-"Day2_finger.hard_LI"
demographics <- makeLI(demographics,origdat,task,mycolname,polarity,mycutoff)

cor1 <- cor(demographics$Day1_finger.easy_LI,demographics$Day2_finger.easy_LI)
plot(demographics$Day1_finger.easy_LI,demographics$Day2_finger.easy_LI,main='Finger easy LI')
abline(v=0)
abline(h=0)
text(40,10,paste0('r = ',round(cor1,3)))
cor2 <- cor(demographics$Day1_finger.hard_LI,demographics$Day2_finger.hard_LI)
plot(demographics$Day1_finger.hard_LI,demographics$Day2_finger.hard_LI,main='Finger hard LI')
abline(v=0)
abline(h=0)
text(40,10,paste0('r = ',round(cor2,3)))
