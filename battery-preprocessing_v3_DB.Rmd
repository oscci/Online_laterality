...
for (i in 1:nsub) { # loop through subjects
  ...
  for (j in 1:2) {
    ...
    temp_dat$p.corr[myrow] <- 100*mean(tmp2$Correct,na.rm=TRUE) # add row for %accurate responses - NB for chimeric only error is wrong emotion - measure is just a measure of choice. For RDT, can make error of selecting wrong side, so % correct is meaningful
