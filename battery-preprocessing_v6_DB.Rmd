...
demo_dat <- read.csv("./data/Demographics.csv",stringsAsFactors = F)
...
porta_dat <- read.csv("./data/porta.csv",stringsAsFactors = F)
miles_dat <- read.csv("./data/miles.csv",stringsAsFactors = F)
...
EHI <- read.csv("./data/EHI.csv",stringsAsFactors = F)
...
lexTALE <- read.csv("./data/LexTALE.csv",stringsAsFactors = F)
...
# Set directory
for (d in 1:2){
  data_file <- "./data/combined/day1_combined.csv" #"https://osf.io/mhk29/download" #day 1 data
  
  if (d ==2){
    data_file <-  "./data/combined/day2_combined.csv"#"https://osf.io/ebm6s/download"
  }
...
allsum <- merge(allsum,chimdatL1,by="subject")
allsum <- merge(allsum,chimdatR1,by="subject")
allsum <- merge(allsum,chimdatL2,by="subject")
allsum <- merge(allsum,chimdatR2,by="subject")
myn <-ncol(allsum)
...
#Add to allsum
allsum <- merge(allsum,DLdatL1,by="subject")
allsum <- merge(allsum,DLdatR1,by="subject")
allsum <- merge(allsum,DLdatL2,by="subject")
allsum <- merge(allsum,DLdatR2,by="subject")
myn <-ncol(allsum)
...
allsum <- merge(allsum,pcorNull,by="subject")
}
...
allsum <- merge(allsum,RDTdatL1,by="subject")
allsum <- merge(allsum,RDTdatR1,by="subject")
allsum <- merge(allsum,RDTdatL2,by="subject")
allsum <- merge(allsum,RDTdatR2,by="subject")
myn <-ncol(allsum)
...
allsum$tlat_RDT_1[w]<-NA
w<-which(is.na(allsum$Day2.RDT_RT_LI))
allsum$tlat_RDT_2[w]<-NA
...
allsum <- merge(allsum,fingerL1,by="subject")
allsum <- merge(allsum,fingerR1,by="subject")
allsum <- merge(allsum,fingerL2,by="subject")
allsum <- merge(allsum,fingerR2,by="subject")
myn <-ncol(allsum)
...
```{r Chimeric Split Half}
varlist <-   c("subject", "side", "accurate", "RT") #need to be in this order, ie sub, side, acc and RR
latlist <- c("Left","Right") #names of factor levels
nloop=2 #same as levels of latlist
# create data frames
CF_dat1$split <- c("odds", "even")
  CF_dat_1.1 <- CF_dat1[CF_dat1$split== "odds",]
  CF_dat_1.2 <- CF_dat1[CF_dat1$split== "even",]
CF_dat2$split <- c("odds", "even")
  CF_dat_2.1 <- CF_dat2[CF_dat2$split== "odds",]
  CF_dat_2.2 <- CF_dat2[CF_dat2$split== "even",]
# create aggregate data
chim_dat1.1 <- make.df(CF_dat_1.1,varlist,latlist,nloop)
chim_dat1.2 <- make.df(CF_dat_1.2,varlist,latlist,nloop)
chim_dat2.1 <- make.df(CF_dat_2.1,varlist,latlist,nloop)
chim_dat2.2 <- make.df(CF_dat_2.2,varlist,latlist,nloop)
#exclude
#find included cases
w<-which(allsum$exclude==0)
  inclsubs <- allsum$subject[w]
...
origdat<-chim_dat1.2
origdat$X1 <- origdat$N
mycolname<-"split.chim.1.2"
polarity<- (1)#so LI is positive in predicted direction
allsum <- makeZ(allsum,origdat,task,mycolname,polarity,mycutoff)
# 2 odds
origdat<-chim_dat2.1
origdat$X1 <- origdat$N
mycolname<-"split.chim.2.1"
polarity<- (1)#so LI is positive in predicted direction
allsum <- makeZ(allsum,origdat,task,mycolname,polarity,mycutoff)
# 2 evens
origdat<-chim_dat2.2
origdat$X1 <- origdat$N
mycolname<-"split.chim.2.2"
polarity<- (1)#so LI is positive in predicted direction
allsum <- makeZ(allsum,origdat,task,mycolname,polarity,mycutoff)
# remove
w<-which(allsum$exclude == 1)
  allsum$split.chim.1.1[w] <- NA
  allsum$split.chim.1.2[w] <- NA
  allsum$split.chim.2.1[w] <- NA
  allsum$split.chim.2.2[w] <- NA

print(paste0("Chimeric faces session 1 split-half: ", cor(allsum$split.chim.1.1, allsum$split.chim.1.2, use = "complete.obs")))
print(paste0("Chimeric faces session 2 split-half: ", cor(allsum$split.chim.2.1, allsum$split.chim.2.2, use = "complete.obs")))
```

#### Dichotic listening

```{r Dichotic Split Half}
varlist <-   c("subject", "side", "accurate", "RT") #need to be in this order, ie sub, side, acc and RR
origdat <- dichotic_dat1
latlist <- c("Left","Right") #names of factor levels
nloop=2 #same as levels of latlist
# made data frames
dichotic_dat1$split <- c("odds", "even")
  DL_1.1 <- dichotic_dat1[dichotic_dat1$split== "odds",]
  DL_1.2 <- dichotic_dat1[dichotic_dat1$split== "even",]
dichotic_dat2$split <- c("odds", "even")
  DL_2.1 <- dichotic_dat2[dichotic_dat2$split== "odds",]
  DL_2.2 <- dichotic_dat2[dichotic_dat2$split== "even",]
# create aggregate data
DL_dat1.1 <- make.df(DL_1.1,varlist,latlist,nloop)
DL_dat1.2 <- make.df(DL_1.2,varlist,latlist,nloop)
DL_dat2.1 <- make.df(DL_2.1,varlist,latlist,nloop)
DL_dat2.2 <- make.df(DL_2.2,varlist,latlist,nloop)
# exclude
w<-which(is.na(DL_dat1.1$subject))
  if (length(w)>0) {DL_dat1.1<-DL_dat1.1[-w,]}
w<-which(is.na(DL_dat1.2$subject))
  if (length(w)>0) {DL_dat1.2<-DL_dat1.2[-w,]}
w<-which(is.na(DL_dat2.1$subject))
  if (length(w)>0) {DL_dat2.1<-DL_dat2.1[-w,]}
w<-which(is.na(DL_dat2.2$subject))
  if (length(w)>0) {DL_dat2.2<-DL_dat2.2[-w,]}
DL_dat1.1[is.na(DL_dat1.1)] <- 0
DL_dat1.2[is.na(DL_dat1.2)] <- 0
DL_dat2.1[is.na(DL_dat2.1)] <- 0
DL_dat2.2[is.na(DL_dat2.2)] <- 0
...
origdat<-DL_dat1.2
origdat$X1 <- origdat$N
mycolname<-"split.DL.1.2"
allsum <- makeZ(allsum,origdat,task,mycolname,polarity,mycutoff)
# day 2 odds
origdat<-DL_dat2.1
origdat$X1 <- origdat$N
mycolname<-"split.DL.2.1"
allsum <- makeZ(allsum,origdat,task,mycolname,polarity,mycutoff)
# day 2 evens
origdat<-DL_dat2.2
origdat$X1 <- origdat$N
mycolname<-"split.DL.2.2"
allsum <- makeZ(allsum,origdat,task,mycolname,polarity,mycutoff)
#remove
# remove
w<-which(allsum$exclude == 2)
  allsum$split.DL.1.1[w] <- NA
  allsum$split.DL.1.2[w] <- NA
  allsum$split.DL.2.1[w] <- NA
  allsum$split.DL.2.2[w] <- NA

print(paste0("Dichotic listening session 1 split-half: ", cor(allsum$split.DL.1.1, allsum$split.DL.1.2, use = "complete.obs")))
print(paste0("Dichotic listening session 2 split-half: ", cor(allsum$split.DL.2.1, allsum$split.DL.2.2, use = "complete.obs")))
```
...
nloop<-length(latlist)
# made data frames
RDT_dat1$split <- c("odds", "even")
  RDT_dat1.1 <- RDT_dat1[RDT_dat1$split== "odds",]
  RDT_dat1.2 <- RDT_dat1[RDT_dat1$split== "even",]
RDT_dat2 <- RDT_dat2[-nrow(RDT_dat2),]
RDT_dat2$split <- c("odds", "even")
  RDT_dat2.1 <- RDT_dat2[RDT_dat2$split== "odds",]
  RDT_dat2.2 <- RDT_dat2[RDT_dat2$split== "even",]
# create aggregate data
RDT_df1.1 <- make.df(RDT_dat1,varlist,latlist,nloop)
RDT_df1.2 <- make.df(RDT_dat2,varlist,latlist,nloop)
RDT_df2.1 <- make.df(RDT_dat1,varlist,latlist,nloop)
RDT_df2.2 <- make.df(RDT_dat2,varlist,latlist,nloop)
# remove NaN
is.nan.data.frame <- function(x)
  do.call(cbind, lapply(x, is.nan))
RDT_df1.1[is.nan(RDT_df1.1)] <- NA
RDT_df1.2[is.nan(RDT_df1.2)] <- NA
RDT_df2.1[is.nan(RDT_df2.1)] <- NA
RDT_df2.2[is.nan(RDT_df2.2)] <- NA

# exclude
wn <-unique(which(RDT_df1.1$accurate ==0),which(RDT_df1.2$accurate ==0, (which(RDT_df2.1$accurate ==0, (which(RDT_df2.2$accurate ==0))))))
w1<-which(RDT_df1.1$p.corr < 75)
w2<-which(RDT_df1.2$p.corr < 75)
w3<-which(RDT_df2.1$p.corr < 75)
w4<-which(RDT_df2.2$p.corr < 75)
exRDT<-unique(c(RDT_df1.1$subject[w1],
                RDT_df1.2$subject[w2],
                RDT_df2.1$subject[w3],
                RDT_df2.1$subject[w4],
                RDT_df1.1$subject[wn],
                RDT_df1.2$subject[wn],
                RDT_df2.1$subject[wn],
                RDT_df2.2$subject[wn]))
allsum$exRDT_split <- 0
allsum$exRDT_split[allsum$subject %in% exRDT]<-1 #decided best to have separate column for exclusion for each task
w<-which(allsum$exRDT_split==0)  
incRDT <- allsum$subject[w]
ninc <- length(incRDT)

# MAKE Z LAT
for (d in 1:4){
  ifelse(d==1, myfile<-RDT_dat1.1, 
         ifelse(d==2, myfile<-RDT_dat1.2,
                ifelse(d==3, myfile<-RDT_dat2.1,
                       myfile<-RDT_dat2.2)))
    ifelse(d==1, lab<- 1.1, 
         ifelse(d==2, lab<- 1.2,
                ifelse(d==3, lab<- 2.1,
  ...
  wc<-which(colnames(allsum)=='tempt')
  colnames(allsum)[wc]<-paste0('split.RDT.',lab)
}
w<-which(is.na(allsum$Day1.RDT_RT_LI)) #remove those with NA in LI
  allsum$split.RDT.1.1[w]<-NA
  allsum$split.RDT.1.2[w]<-NA
w<-which(is.na(allsum$Day2.RDT_RT_LI))
  allsum$split.RDT.2.1[w]<-NA
  allsum$split.RDT.2.1[w]<-NA

print(paste0("Rhyme decision session 1 split-half: ", cor(allsum$split.RDT.1.1, allsum$split.RDT.1.2, use = "complete.obs")))
print(paste0("Rhyme decision session 2 split-half: ", cor(allsum$split.RDT.2.1, allsum$split.RDT.2.2, use = "complete.obs")))
```

### list of participants who completed. 

We need a list of participants here who completed the both days for later filtering. 

```{r lister}
# CF
day2.CF <- data.frame(matrix(nrow=length(complete.day2.CF), ncol= 2))
  colnames(day2.CF) <- c("task", "subject")
  day2.CF$subject <- complete.day2.CF
  day2.CF$task <- "CF"
# DL
day2.DL <- data.frame(matrix(nrow=length(complete.day2.DL), ncol= 2))
  colnames(day2.DL) <- c("task", "subject")
  day2.DL$subject <- complete.day2.DL
  day2.DL$task <- "DL"
# RD
day2.RD <- data.frame(matrix(nrow=length(complete.day2.RD), ncol= 2))
  colnames(day2.RD) <- c("task", "subject")
  day2.RD$subject <- complete.day2.RD
  day2.RD$task <- "CF"
# Finger
day2.FT <- data.frame(matrix(nrow=length(complete.day2.FT), ncol= 2))
  colnames(day2.FT) <- c("task", "subject")
  day2.FT$subject <- complete.day2.FT
  day2.FT$task <- "CF"
# now create a combined df
complete.day2a <- merge(day2.CF, day2.DL, by= "subject")
complete.day2b <- merge(day2.RD, day2.FT, by= "subject")
complete.day2 <- merge(complete.day2a, complete.day2b, by= "subject")
# add row to code
complete.day2$day2.complete <- 1
complete.day2 <- dplyr::select(complete.day2, -2, -3, -4, -5)
# now merge into allsum
allsum <- merge(allsum, complete.day2, by= "subject", all= TRUE)

# now write this for filtering
write.csv(allsum, "./data/allsum.csv")
```

### data frames for general plots

In the MS we add a section on raw scores (simple plots). To do this, we require a data frame for each measure. 
...
RDT_df1$task <- "RD"
# merge and add day
plot_data_1 <- rbind(chim_dat1, DL_dat1, RDT_df1[RDT_df1$side != "no rhyme",])
  plot_data_1$Session<- 1
...
RDT_df2$task <- "RD"
# merge and add day
plot_data_2 <- rbind(chim_dat2, DL_dat2, RDT_df2[RDT_df2$side != "no rhyme",])
  plot_data_2$Session<- 2
  
# merge both
plot.data <- rbind(plot_data_2, plot_data_1)

# remove those who only did day 1
plot.data <- merge(plot.data, complete.day2, by= "subject")

# make look up for lextale and remove
lexLOOK <- allsum %>% dplyr::select("subject", "lexTALE")
lexLOOK <- lexLOOK[lexLOOK$lexTALE >= 80,]
# remove those wuth poor lextale
plot.data <- merge(plot.data, lexLOOK, by= "subject")

# now look up exclusions 
excludeLOOK <- allsum %>% dplyr::select("subject", "exclude", "exRDT")
plot.data <- merge(plot.data, excludeLOOK, by= "subject")

# make style conistent
plot.data$side <- tolower(plot.data$side)

# save data frame
write.csv(plot.data, "./data/for_figs.csv")
...
CF.cor.plot <- ggplot(CF.corrs, aes(x=trial, y=abs(cor))) +
  geom_hline(yintercept=.65, linetype="dashed", color = "red") +
  geom_line(size=1, alpha=0.8) + ylim(0, 1) + theme_bw() +
  xlab("Number of trials") + ylab("Test-retest reliability") + ggtitle("(B)") + theme(plot.title = element_text(hjust = 0.5))
...
DL.cor.plot <- ggplot(DL.corrs, aes(x=trial, y=abs(cor))) +
  geom_hline(yintercept=.65, linetype="dashed", color = "red") +
  geom_line(size=1, alpha=0.8) + ylim(0, 1) + theme_bw() +
  xlab("Number of trials") + ylab("Test-retest reliability") + ggtitle("(A)") + theme(plot.title = element_text(hjust = 0.5))
# print number of trials needed
