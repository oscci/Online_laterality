...
chimdatL1<-select(filter(chim_dat1,side=='Left'),subject,accurate)
chimdatR1<-select(filter(chim_dat1,side=='Right'),subject,accurate)
chimdatL2<-select(filter(chim_dat2,side=='Left'),subject,accurate)
chimdatR2<-select(filter(chim_dat2,side=='Right'),subject,accurate)
...
```{r add.dichotic.raw}
#added by DB 17th July 2020: include raw accuracy L and R in the allsum dataframe
DLdatL1<-select(filter(DL_dat1,side=='Left'),subject,accurate)
DLdatR1<-select(filter(DL_dat1,side=='Right'),subject,accurate)
DLdatL2<-select(filter(DL_dat2,side=='Left'),subject,accurate)
DLdatR2<-select(filter(DL_dat2,side=='Right'),subject,accurate)
...
```{r add.rdt.raw}
#added by DB 17th July 2020: include raw accuracy L and R in the allsum dataframe
RDTdatL1<-select(filter(RDT_df1,side=='left'),subject,RT)
RDTdatR1<-select(filter(RDT_df1,side=='right'),subject,RT)
RDTdatL2<-select(filter(RDT_df2,side=='left'),subject,RT)
RDTdatR2<-select(filter(RDT_df2,side=='right'),subject,RT)
...
for (d in 1:2){
  ...
  myfile<-filter(myfile,side %in% c('left','right'))
  ...
  for (i in 1:nsubs) {
    ...
    if(length(w)>10)
    {
      myt<-t.test(myfile$RT[w]~myfile$side[w]) #this is based on logRT
      allsum$tlat_RDT[i]<-myt$statistic
    }
  }
  if (d==1){
    maxcol <-length(colnames(allsum))
    colnames(allsum)[maxcol]<-c('tlat_RDT_1')
  }
  if (d==2){
    maxcol <-length(colnames(allsum))
    colnames(allsum)[maxcol]<-c('tlat_RDT_2')
  }
}
...
allsum[w,(colcount-1):colcount]<-NA
...
```{r add.fingerL_R}
#added by DB 17th July 2020: include raw accuracy L and R in the allsum dataframe
fingerL1<-select(finger.df,subject,L1)
fingerR1<-select(finger.df,subject,R1)
fingerL2<-select(finger.df,subject,L2)
fingerR2<-select(finger.df,subject,R2)
