...
demo_dat <- read_csv("./data/Demographics.csv", na = c("empty", "NA")) # read in data
# relabel 
...
miles_dat <- read_csv("./data/Miles.csv", na = c("empty", "NA")) # read in data
porta_dat <- read_csv("./data/Porta.csv", na = c("empty", "NA")) # read in data
# add test ID
...
```{r EHI, warning=FALSE}
EHI <- read_csv("./data/EHI.csv", na = "empty") # read in data
...
# score responses according to the original scoring method in Oldifeld (1971)
EHI$right_hand <- 0
EHI$left_hand <- 0
for (r in 1:nrow(EHI)) {
  if(EHI$Response[r] == 1){
...
# merge with allsum
allsum <- merge(allsum, EHI_index, by= "subject")

#now plot EHI in relation to handedness (modified by DB)

eplot<-ggplot(allsum, aes(handpch,index_EHI, colour=gender)) +
  geom_jitter(width=.25,height=2)+
  ggtitle("Handedness index on the Edinburgh Handedness Inventory")
eplot
#This is not perfect, but gives and idea to check EHI is valid etc.

ggplot(allsum, aes(x=index_EHI)) + geom_density(aes(group=handpch,fill=handpch)) + theme_classic()

#NB one supposed right-hander has index_EHI around -50. 
#I will recode this one, as this seems an error.
...
# read data
lexTALE <- read_csv("./data/LexTALE.csv", na = "empty")
# filter
...
# create data frame
nsub <-length(levels(lexTALE$subject))
...
ggplot(allsum, aes(x=lexTALE)) + geom_density(aes(group=handpch,fill=handpch)) + theme_classic()
...
Before doing anything with the data, we now pull in all the data from all nodes and combine them from a sub folder in the directory. Participants, identified by random letter strings, are stacked on top of each other. We start by reading the data and cutting out unwanted information.

```{r readraw,include= FALSE, warning=FALSE}
# Set directory
for (d in 1:2){
  data_file <- "./data/combined/day1_combined.csv"
  if (d ==2){
    data_file <- "./data/combined/day2_combined.csv"
  }
...
# write as factors
all_dat2$subject <- as.factor(trimws(all_dat2$Participant.Public.ID))
...
makeLI <- function(allsums,origdat,task,mycolname,polarity,mycutoff){
...
# now merge with allsum
allsums <- merge(allsums, LIdf, by= "subject")
lastcol<-length(colnames(allsums))
colnames(allsums)[lastcol]<-mycolname
...
makeZ <- function(allsums,origdat,task,mycolname,polarity,mycutoff){
# we do this for accuracy first
# reformat data frame
keeps <- c("subject", "side", "X1") #X1 holds either accuracy or RT
Zdf <- origdat[keeps]
...
# now merge with allsum
allsums <- merge(allsums, Zdf, by= "subject")
lastcol<-length(colnames(allsums))
colnames(allsums)[lastcol]<-mycolname
...
# mark Ps with too many trials indicating issues with the task or cases where less than 80% of data is left
# we mark this to exclude in the next section
out2 <- subset(data.frame(outliertable), Var2== 0)
out2 <- out2[out2$Freq > 96 | out2$Freq < 76,]
...
# bind one long list of those who have missing trials or too much data
CF_out <- rbind(out1, out2)
```
...
emotion_mean$subject <- as.factor(trimws(as.character(emotion_mean$subject)))
# add this to allsum 
allsum <- merge(allsum, emotion_mean, by= "subject")
}
mycol <- length(colnames(allsum))
colnames(allsum)[(mycol-1):mycol]<-c('Emot.recog1','Emot.recog2')

plot(jitter(allsum$Emot.recog1,10),jitter(allsum$Emot.recog2,10),pch=3,cex=.6)
abline(h=.75)
abline(v=.75)
...
w <- union(which(allsum$Emot.recog1 < .75),which(allsum$Emot.recog2 < .75))
allsum$exclude[w]<-1 #code exclude = 1 if excluded because poor emot recog
# now mark those with too many or too few
allsum$exclude[allsum$subject %in% CF_out$Var1] <- 100
...
chim_dat1[is.na(chim_dat1)] <- 0
chim_dat2[is.na(chim_dat2)] <- 0
```
...
plot(allsum$Day1_CF_RT_LI,allsum$Day2_CF_RT_LI,main='Chimeric LI RT',col=(1+allsum$exclude))
abline(v=0)
abline(h=0)
text(-10,22,paste0('r = ',round(cor2,3)))
...
As we can see, we generally get a left side/right hemisphere advantage in our accuracy data. We would not expect a RT advantage for this type of measure, where the response is to a single stimulus with conflicting information from the 2 sides.
...
# remove those with too many trials indicating issues with the task
# we mark this to exclude in the next section
out2 <- subset(data.frame(outliertable), Var2== 0)
out2 <- out2[out2$Freq > 120 | out2$Freq < 96,]
...
# bind one long list of those who have missing trials or too much data
DL_out <- rbind(out1, out2)
```

First, we judge participants' ability to correctly identify sounds using the "dichotic_same" dataframe. Participants are marked as excluded if their performance is below 75%.
...
sound_mean$subject <- as.factor(trimws(as.character(sound_mean$subject)))
# add this to allsum 
allsum <- merge(allsum, sound_mean, by= "subject")
}
mycol <- length(colnames(allsum))
colnames(allsum)[(mycol-1):mycol]<-c('sound.recog1','sound.recog2')
...
# remove those with too many or missing trials
allsum$exclude[allsum$subject %in% DL_out$Var1] <- 100
# table to summarise
t <- table(allsum$exclude) 
rownames(t)<-c('include','exclude CF','exclude dich','exclude CF+dich','trials')
t
```
...
cor1 <- cor(allsum$Day1_DL_acc_LI,allsum$Day2_DL_acc_LI, use = "complete.obs", method = "pearson")
...
cor2<-cor(allsum$Day1_DL_RT_LI,allsum$Day2_DL_RT_LI, use = "complete.obs")
...
mycutoff <- 10 #NB! need to check against prereg. I think this is N correct, rather than percentage. People with avg scores below this value will be excluded.AP: chekced against pre-reg
polarity<- (-1)#so LI is positive in predicted direction
allsum <- makeZ(allsum,origdat,task,mycolname,polarity,mycutoff)
...
# remove those with too many trials indicating issues with the task
out2 <- subset(data.frame(outliertable), Var2== 0)
out2 <- out2[out2$Freq > 260 | out2$Freq < 208,]
...
# bind one long list of those who have missing trials or too much data
RDT_out <- rbind(out1, out2)

# once again, mark rather than remove those with poor performance on identical stimuli
allsum$exclude[allsum$subject %in% RDT_out$Var1] <- 100
# table to summarise
t <- table(allsum$exclude) 
rownames(t)<-c('include','exclude CF','exclude dich','exclude CF+dich','trials')
t
...
w<-which(is.na(allsum$RDT_acc_LI))
if(length(w)>0)
{allsum$RDT_RT_LI[w]<-NA}
#t.test to test if laterality index differs from zero
t.test(allsum$RDT_p.corr_LI)
t.test(allsum$RDT_RT_LI)
#rename column to identify day
myncol <- length(colnames(allsum))
colnames(allsum)[(myncol-1):myncol] <- paste0('Day',d,'.',colnames(allsum)[(myncol-1):myncol])

}
cor1<-cor(allsum$Day1.RDT_p.corr_LI,allsum$Day2.RDT_p.corr_LI,use='complete.obs')
cor2<-cor(allsum$Day1.RDT_RT_LI,allsum$Day2.RDT_RT_LI,use='complete.obs')
plot(allsum$Day1.RDT_p.corr_LI,allsum$Day2.RDT_p.corr_LI,main='Rhyme detect: LI for accuracy (%correct)')
abline(v=0)
abline(h=0)
text(-3,3,paste0('r = ',round(cor1,3)))
plot(allsum$Day1.RDT_RT_LI,allsum$Day2.RDT_RT_LI,main='Rhyme detect: LI for RT')
abline(v=0)
abline(h=0)
...
for (d in 1:2){
  ...
  allsum$tlat_RDT<-NA
   allsum$plat_RDT<-NA
  for (i in 1:nsubs) {
    w<-which(myfile$subject==allsum$subject[i])
    if(length(w)>0)
    {
      myt<-t.test(myfile$RT[w]~myfile$side[w])
      allsum$tlat_RDT[i]<-myt$statistic
      allsum$plat_RDT[i]<-myt$p.value
    }
  }
   if (d==1){
     maxcol <-length(colnames(allsum))
     colnames(allsum)[(maxcol-1):maxcol]<-c('tlat_RDT_1','plat_RDT_1')
   }
   if (d==2){
     maxcol <-length(colnames(allsum))
     colnames(allsum)[(maxcol-1):maxcol]<-c('tlat_RDT_2','plat_RDT_2')
   }
}
colcount<-length(colnames(allsum))
# Remove values for cases that are excluded for low acc: do this for both times by just blitzing last 4 columns of allsum
w<-which(is.na(allsum$Day1.RDT_RT_LI))
allsum[w,(colcount-3):colcount]<-NA
w<-which(is.na(allsum$Day2.RDT_RT_LI))
allsum[w,(colcount-3):colcount]<-NA

# compare t-value with LI : use t = 1.65 as cutoff for sig lateralised
 plot(allsum$tlat_RDT_1,allsum$Day1.RDT_RT_LI)
 abline(v=1.65)
  plot(allsum$tlat_RDT_2,allsum$Day2.RDT_RT_LI)
   abline(v=1.65)
# compare t-value from day 1 and day 2 
     myr <- cor(allsum$tlat_RDT_1,allsum$tlat_RDT_2,use='complete.obs')
    plot(allsum$tlat_RDT_1,allsum$tlat_RDT_2)
    abline(v=0)
    abline(h=0)
   abline(v=1.65) 
   abline(h=1.65)
...
finger <- read.csv("data/combined/day1_combined.csv", na.strings = "NA")
# cleaning
...
finger <- read.csv("data/combined/day2_combined.csv", na.strings = "NA")
# cleaning
finger <- finger[finger$Attempt >= 1,] # only answers 
finger <- finger[finger$display == "right hand" | finger$display == "left hand",] # only for task
# write as factors
...
cor1 <- cor(allsum$Day1_Finger_acc_Z,allsum$Day2_Finger_acc_Z)
plot(allsum$Day1_Finger_acc_Z,allsum$Day2_Finger_acc_Z,main='Finger Z accuracy')
abline(v=0)
abline(h=0)
...
# comment out to get participants with full data set
allsum <- allsum[allsum$exclude == 0,]
...
ggplot(data=a, aes(x=handedness, y=n, fill= handedness)) +
  geom_bar(stat="identity", alpha= .5, colour="black")+
  geom_text(aes(label=n), vjust=1.6, color="white", size=3.5)+
  theme_minimal() + ggtitle("Handedness") + 
  ylab("Count") + xlab("")
# third, show footedness
a <- allsum %>%
...
mycols <- c('Day1_CF_acc_LI', 'Day1_CF_acc_Z','Day2_CF_acc_LI', 'Day2_CF_acc_Z',
            'Day1_DL_acc_LI', 'Day1_Dich_acc_Z','Day2_DL_acc_LI', 'Day2_Dich_acc_Z',
            'Day1.RDT_RT_LI', 'tlat_RDT_1', 'Day2.RDT_RT_LI', 'tlat_RDT_2',
            'Day1_finger_LI','Day1_Finger_acc_Z', 'Day2_finger_LI','Day2_Finger_acc_Z')
colnums<- which(colnames(allsum) %in% mycols)
M <- allsum[,colnums]
M <- M %>% 
  rename(
    "CF (z) 1" = Day1_CF_acc_Z,
    "CF (z) 2" = Day2_CF_acc_Z,
    "DL (z) 1" = Day1_Dich_acc_Z,
    "DL (z) 2" = Day2_Dich_acc_Z,
    "RD (t) 1" = tlat_RDT_1,
    "RD (t) 2" = tlat_RDT_2,
    "FT (z) 1" = Day1_Finger_acc_Z,
    "FT (z) 2" = Day2_Finger_acc_Z,
    "CF (LI) 1" = Day1_CF_acc_LI,
    "CF (LI) 2" = Day2_CF_acc_LI,
    "DL (LI) 1" = Day1_DL_acc_LI,
    "DL (LI) 2" = Day2_DL_acc_LI,
    "RD (LI) 1" = Day1.RDT_RT_LI,
    "RD (LI) 2" = Day2.RDT_RT_LI,
    "FT (LI) 1" = Day1_finger_LI,
    "FT (LI) 2" = Day2_finger_LI)
# PLOT TO SEE IF DIFFERENT FROM ZERO
# rearrange df
N <- M %>% select("CF (LI) 1", "CF (LI) 2", "CF (z) 1", "CF (z) 2",
             "DL (LI) 1", "DL (LI) 2", "DL (z) 1", "DL (z) 2",
             "RD (LI) 1", "RD (LI) 2", "RD (t) 1", "RD (t) 2",
             "FT (LI) 1", "FT (LI) 2", "FT (z) 1", "FT (z) 2")
# rearrange for pirate plot
# set participant ID
N$ID <- 1:nrow(N)
# format to long
data_long <- tidyr::gather(N, task, index, `CF (LI) 1`:`FT (z) 2`, factor_key=TRUE)
# add task identifer, session, etc
data_long <- data_long %>%
    mutate(group= if_else(task == "CF (LI) 1" | task == "CF (LI) 2" | task == "DL (LI) 1" | task == "DL (LI) 2" |
                          task == "RD (LI) 1" | task == "RD (LI) 2" | task == "FT (LI) 1" | task == "FT (LI) 2",
                          "LI", "t/z"),
           session= if_else(task == "CF (LI) 1" | task == "CF (z) 1" |
                            task == "DL (LI) 1" | task == "DL (z) 1" |
                            task == "RD (LI) 1" | task == "RD (t) 1" |
                            task == "FT (LI) 1" | task == "FT (z) 1", "1", "2"),
           task2= if_else(task == "CF (LI) 1" | task == "CF (LI) 2" | task == "CF (z) 1" | task == "CF (z) 2", "Chimeric faces",
                  if_else(task == "DL (LI) 1" | task == "DL (LI) 2" | task == "DL (z) 1" | task == "DL (z) 2", "Dichotic listening",
                  if_else(task == "RD (LI) 1" | task == "RD (LI) 2" | task == "RD (t) 1" | task == "RD (t) 2", "Rhyme decision",
                  "Finger Tapping"))))
# PLOT CORRELATION BETWEEN DAYS
# create matrix
M<-cor(M)
# removed odds and evens on each axis for plotting
dat <- M
toDeleteA <- seq(2, nrow(M), 2)
toDeleteB <- seq(1, ncol(M), 2)
dat <- dat[ toDeleteA , toDeleteB ]
dat=dat[,order(ncol(dat):1)]
# plot
ggcorrplot::ggcorrplot(dat, lab = TRUE, 
                       ggtheme = ggplot2::theme_minimal,
                       colors = c("#6D9EC1", "white", "#E46726"),
                       show.legend = FALSE, digits = 2, 
                       title = "Correlation matrix for LIs on the four behavioural laterality tasks")

# correlate each
# dichotic
DL_scatter <- ggplot(allsum, aes(x= Day1_DL_acc_LI, y= Day2_DL_acc_LI)) +
  ...
  annotate("text", x = 90, y = 90, label = "paste(italic(R), \" = .72\")", parse = TRUE) +
  theme(legend.position = c(.15, .90))
# Set relative size of marginal plots (main plot 10x bigger than marginals)
ggExtra::ggMarginal(DL_scatter, groupColour = TRUE, groupFill = TRUE)
# chimeric
CF_scatter <- ggplot(allsum, aes(x= Day1_CF_acc_LI, y= Day2_CF_acc_LI)) +
  geom_hline(yintercept=0, color = "black") +
  geom_vline(xintercept=0, color = "black") +
  geom_point(aes(color= handedness)) +
  geom_rug(aes(color= handedness)) +
...
# rhyme
rhyme_scatter <- ggplot(allsum, aes(x= Day1.RDT_RT_LI, y= Day2.RDT_RT_LI)) +
  geom_hline(yintercept=0, color = "black") +
  geom_vline(xintercept=0, color = "black") +
  geom_point(aes(color= handedness)) + 
  geom_rug(aes(color= handedness)) +
  ...
  annotate("text", x = 8, y = 8, label = "paste(italic(R), \" = .63\")", parse = TRUE) +
  theme(legend.position = c(.15, .90))
# Set relative size of marginal plots (main plot 10x bigger than marginals)
ggExtra::ggMarginal(rhyme_scatter, groupColour = TRUE, groupFill = TRUE)
# finger
finger_scatter <- ggplot(allsum, aes(x= Day1_finger_LI, y= Day2_finger_LI)) +
  geom_hline(yintercept=0, color = "black") +
  geom_vline(xintercept=0, color = "black") +
  geom_point(aes(color= handedness)) +
  geom_rug(aes(color= handedness)) +
  ...
  annotate("text", x = 18, y = 18, label = "paste(italic(R), \" = .89\")", parse = TRUE) +
  theme(legend.position = c(.15, .90))
# Set relative size of marginal plots (main plot 10x bigger than marginals)
